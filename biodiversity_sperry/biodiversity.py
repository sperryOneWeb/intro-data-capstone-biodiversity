...
contingency = [[mammal_protected, mammal_not_protected],
               [bird_protected, bird_not_protected]]
...
print "p = %s" % chi2_contingency(contingency)[1]
...
contingency = [[mammal_protected, mammal_not_protected],
               [reptile_protected, reptile_not_protected]]
...
for group in species_categories:
    ...
    contingency = [[mammal_protected, mammal_not_protected],
               [group_protected, group_not_protected]]
    p = chi2_contingency(contingency)[1]
    
    index = relation_to_mammals[relation_to_mammals['Category']==group].index.item()
    relation_to_mammals.at[index, 'P Value'] = p

relation_to_mammals.head(7)
...
print "Number of species: %s" % observations.scientific_name.nunique()
print "Number of Entries: %s" % observations.scientific_name.count()
observations.head()
...
park_locations = observations.park_name.unique() 
print park_locations
print observations.groupby('park_name').observations.sum()
plt.pie(observations.groupby('park_name').observations.sum(), labels=park_locations, autopct='%d%%')
plt.axis('equal')
plt.title("Sightings per Park")
...
num_sheep_results = species[(species['is_sheep'] == True)].scientific_name.nunique()
print num_sheep_results
sheep_results = species[(species['is_sheep'] == True)]
sheep_results
...
afflicated_pop = 0.15
target_effect = 0.05
minimum_detectable_effect = 100 * (target_effect / afflicated_pop)
...
obs_by_park['weeks_of_study'] = sample_size_per_variation / obs_by_park['observations']
obs_by_park
